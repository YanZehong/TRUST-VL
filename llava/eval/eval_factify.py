...
def read_data(path):
    if '.jsonl' in path:
        ...
        df_sub = df[(df['Category']=='Refute') | (df['Category']=='Support_Multimodal')]
        df_sub = df_sub.reset_index()
        data_output = []
        for index, row in df_sub.iterrows():
            cur_q = {}
            orig_id = str(row['index'])
            cur_id = str(row['index']) + '-' + row['Category']
            if row['Category'] == 'Refute':
                claim_dir = f"Factify2/images-val/claim/Refute/{orig_id}.jpg"
                document_dir = f"Factify2/images-val/document/Refute/{orig_id}.jpg"
            else:
                claim_dir = f"Factify2/images-val/claim/Support_Multimodal/{orig_id}.jpg"
                document_dir = f"Factify2/images-val/document/Support_Multimodal/{orig_id}.jpg"
            
            image_path = os.path.join("/home/zehong/LLaVA/data", claim_dir)
            news_text = row['claim'] 
            textual_evidence = row['document']
        
            cur_q['question_id'] = cur_id
            cur_q['news_text'] = news_text
            cur_q['image_path'] = claim_dir
            cur_q['textual_evidence'] = textual_evidence
            cur_q['visual_evidence'] = document_dir 
            cur_q["fake_cls"] = row["Category"]

            if row['Category'] == 'Refute':
                cur_q["reference"] = 'fake'
            else:
                cur_q["reference"] = 'real'
            data_output.append(cur_q)
    else:
        print("Cannot handle input format.")
    return data_output
...
def main(args):
    ...
    for i, row in enumerate(tqdm(questions)):
        ...
        new_dict = {}
        row['idx'] = str(row['question_id']) + '-' + row['fake_cls']
        row['caption'] = row['news_text']

        caption = row['caption']
        image_file = os.path.join(args.image_folder, row['image_path'].replace("/home/zehong/data_misinformation/", ""))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--question-file", type=str, default="./data/eval/Factify2_3000.jsonl")
    parser.add_argument("--image-folder", type=str, default="/home/zehong/LLaVA/data")
    parser.add_argument("--answers-file", type=str, default="./outputs/eval/answer_factify2.jsonl")
