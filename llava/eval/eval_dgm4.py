...
from llava.conversation import conv_templates as conv_templates_news
...
def eval_model(args, tokenizer, model, image_processor, context_len):
    ...
    if 'trustvl' in args.model_path:
        conv = conv_templates_news[args.conv_mode].copy()
    else:
...
def main(args):
    ...
    for i, row in enumerate(tqdm(questions)):
        ...
        caption = row['text']
        image_file = os.path.join(args.image_folder, row['image'])
        ...
        if 'trustvl' in model_path:
            ...
            system_prompt = "Task description: some rumormongers intentionally manipulate images to make multimodal misinformation. Given a news text and a news image, you are responsible for judging whether the given image is credible and unmanipulated. You will be presented with a text and an image. Based on your assessment of the image’s authenticity, you should choose one of the following responses as the output: \n- 'No' if the image is original with no manipulation to the face or facial expressions. \n- 'Yes' if you find that the face in the image has been manipulated. \n"
        ...
        new_dict['question_id'] = str(row['id']) + row['fake_cls']
        new_dict['news_text'] = row['text']
        new_dict['image_path'] = row['image']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--question-file", type=str, default="./data/eval/DGM4-Face_1000.jsonl")
    parser.add_argument("--image-folder", type=str, default="/home/zehong/LLaVA/data/")
    parser.add_argument("--answers-file", type=str, default="./outputs/eval/answer_dgm4.jsonl")
